...
public class UserApiHandler implements HttpHandler {
    ...
    // 실제로는 DB에 저장한다. 지금은 메모리 리스트로 대신 한다. 즉, 서버를 껏다 켜면 내용이 다 사라진다.
     ...
     *
     */
    private ArrayList<User> copyLUserList() {
        return new ArrayList<>(userList);
    }

    /**
     * GET : 사용자 목록을 조회 (HTTP 메세지 body 없음)
    ...
    private void handleGet(HttpExchange exchange) throws IOException{
        // sendJSON 임 !
        SimpleHttpServer.sendJSON(exchange,200,copyLUserList());
    }
    ...
    private void handlePost(HttpExchange exchange) throws IOException {
        ...
        if (user == null || user.getName() == null || user.getName().isBlank()) {
            SimpleHttpServer.sendResponse(exchange, 400, SimpleHttpServer.TYPE_TEXT, "name 은 반드시 있어야 합니다");
            return;
        }
        ...
        // 5. 등록 성공은 200 대신 201 로 Created 로 응답을 한다.
