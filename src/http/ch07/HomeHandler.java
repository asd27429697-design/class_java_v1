...
public class HomeHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        try {
            // / <-- 이 경로를 처리하는 핸들러
            // 주의할 점
            // "/" 로 등록된 핸들러는 다른 핸들러가 맡지 않은 "모든" 경로를 받아요
            ...
            // Content-Type 을 text/html 로 보내야 브라우저가 HTML 로 해석한다
            // text/palin 으로 보내면 태그가 글자로 보인다 (확인)
            // SimpleHttpServer.sendResponse(exchange, 200,"text/palin; charset=UTF-8", HOME_PAGE);

            SimpleHttpServer.sendResponse(exchange, 200, "text/html; charset=UTF-8", HOME_PAGE);
        } finally {
            exchange.close();
        }
        // MIME 타입 // 컨텐트 타입은 왠만하면 무조건 알기
    }
}
